...
- Displays status and access instructions for all deployed components
...
| deploy.sh | Deploy Elastic Stack | ❌ No | ✅ Yes | ✅ Yes | ✅ Yes |
...
4. cd ../helm_charts
   ./deploy.sh
   → Deploys Elastic Stack (Elasticsearch, Kibana, Logstash) to Kubernetes
...
3. ./deployment_infrastructure/uninstall-k3s-complete.sh
...
│   ├── install-k3s-airgap.sh       ← Phase 2: k3s installation
│   ├── uninstall-k3s-complete.sh   ← Cleanup: k3s uninstall
...
│   │
│   └── helm-files/                 (Generated by collect-all.sh)
│       └── helm                    ← Helm binary
│
...
└── helm_charts/                     ← Phase 4: Kubernetes deployment
    ├── deploy.sh                   ← Deploy Elastic Stack (interactive)
